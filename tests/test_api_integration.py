...
from api_integration.metrics import GitHubMetricsClient
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("user1", 200, {"login": "user1"}, id="200_ok"),
        pytest.param("user2", 404, None,               id="404_none"),
    ]
)
def test_get_github_user_info(mock_github_metrics_client, mocker, username, status_code, expected):
    # patch the requests.get used inside your module
    mock_resp = mocker.MagicMock()
    mock_resp.status_code = status_code
    mock_resp.json.return_value = expected
    mock_get = mocker.patch("api_integration.metrics.requests.get", return_value=mock_resp)
...
def test_get_github_user_repos_pagination(mock_github_metrics_client, mocker, pages, expected_names, expected_calls):
    ...
    mock_get = mocker.patch("api_integration.metrics.requests.get", side_effect=[mockResponse(p) for p in pages])
...
def test_submit_datadog_metric(mock_github_metrics_client, mocker, status_code, expected):
    ...
    mocker.patch("api_integration.metrics.pycurl.Curl", return_value=curl)
...
def test_analyze_developer_activity(mock_github_metrics_client, mocker, username, stars_list, expected):
    mocker.patch("api_integration.metrics.GitHubMetricsClient.get_github_user_info", return_value={"login": username})
    mocker.patch(
        "api_integration.metrics.GitHubMetricsClient.get_github_user_repos",
        return_value=[{"name": f"r{i}", "stars": s, "language": "Py"} for i, s in enumerate(stars_list)]
    )
    submit = mocker.patch("api_integration.metrics.GitHubMetricsClient.submit_datadog_metric", return_value={"status": "success"})
